...
contract ItemsTransferFacet is Modifiers {
  ...
  */
  ...
  function transferFromTokenApproved(
    ...
    if (_fromContract == address(this)) {
      address owner = s.snowdrops[_fromTokenId].owner;
      require(
      ...
      require(s.snowdrops[_fromTokenId].locked == false, "ItemsTransfer: Only callable on unlocked Snowdrops");
    } else {
  ...
  */
  ...
  ///@notice Used to extract items that have been accidentally burned with Snowdrops
  function extractItemsFromSacrificedSnowdrops(
    address _to,
    uint256 _tokenId,
    ...
    Snowdrops storage snowdrops = s.snowdrops[_tokenId];
    require(snowdrops.owner == address(0), "Snowdrops has not been sacrificed");
