...
contract Snowdrops {
  ...
  function balanceOf(address _owner) external view returns (uint256 balance_) {
    require(_owner != address(0), "SnowdropsFacet: _owner can't be address(0)");
  ...
  ///@return snowdropsInfo_ a struct containing all details about
  function getSnowdrops(uint256 _tokenId) external view returns (SnowdropsInfo memory snowdropsInfo_) {
    snowdropsInfo_ = LibSnowdrops.getSnowdrops(_tokenId);
  }
  ...
  function snowdropsClaimTime(uint256 _tokenId) external view returns (uint256 claimTime_) {
    claimTime_ = s.snowdrops[_tokenId].claimTime;
  }
  ...
  function tokenByIndex(uint256 _index) external view returns (uint256 tokenId_) {
    require(_index < s.tokenIds.length, "SnowdropsFacet: index beyond supply");
  ...
  function tokenOfOwnerByIndex(address _owner, uint256 _index) external view returns (uint256 tokenId_) {
    require(_index < s.ownerTokenIds[_owner].length, "SnowdropsFacet: index beyond owner balance");
  ...
  /// @param _owner The address to check for the NFTs
  /// @return snowdropsInfos_ an array of structs,where each struct contains all the details of each NFT
  function allSnowdropsOfOwner(address _owner) external view returns (SnowdropsInfo[] memory snowdropsInfos_) {
    uint256 length = s.ownerTokenIds[_owner].length;
    snowdropsInfos_ = new SnowdropsInfo[](length);
    for (uint256 i; i < length; i++) {
      snowdropsInfos_[i] = LibSnowdrops.getSnowdrops(s.ownerTokenIds[_owner][i]);
    }
  }
  ...
  function ownerOf(uint256 _tokenId) external view returns (address owner_) {
      owner_ = s.snowdrops[_tokenId].owner;
      require(owner_ != address(0), "SnowdropsFacet: invalid _tokenId");
  }
  ...
  function internalTransferFrom(
    ...
    require(_to != address(0), "SnowdropsFacet: Can't transfer to 0 address");
    require(_from != address(0), "SnowdropsFacet: _from can't be 0 address");
    require(_from == s.snowdrops[_tokenId].owner, "SnowdropsFacet: _from is not owner, transfer failed");
    require(
      _sender == _from || s.operators[_from][_sender] || _sender == s.approved[_tokenId],
      "SnowdropsFacet: Not owner or approved to transfer"
    );
    LibSnowdrops.transfer(_from, _to, _tokenId);
  ...
  function tokenURI(uint256 _tokenId) external pure returns (string memory) {
    return LibStrings.strWithUint("https://", _tokenId); //Here is your URL!
  }
}
